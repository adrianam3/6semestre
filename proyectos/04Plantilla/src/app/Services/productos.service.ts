...
export class ProductoService {
  apiurl = 'http://localhost/6semestre/proyectos/03MVC/controllers/productos.controller.php?op=';
  ...
  uno(idProductos: number, idKardex: number): Observable<IProducto> {
    // console.log("id producto    " + idProductos + "   id kardex    " + idKardex);
    const formData = new FormData();
    formData.append('idProductos', idProductos.toString());
    formData.append('idKardex', idKardex.toString());
  ...
  actualizar(producto: IProducto): Observable<string> {
    ...
    // Actualizar el producto y kardex
